...
export default function Broadcast() {
  ...
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          setLoadingMsg(
            "Please enable location services to receive local alerts.",
          );
        },
      );
    } else {
        ...
        if (currentLoc) {
          ...
          if (distance <= newBroadcast.radiusInMeters) {
            ...
            if (Notification.permission === "granted") {
              new Notification("EMERGENCY ALERT: " + newBroadcast.title, {
  ...
  if (!userLocation || loadingMsg) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-(--gl-bg-base) p-4 text-center font-satoshi text-(--gl-text-main)">
        <LuRadioReceiver
          size={48}
          className="text-(--gl-sos-base) animate-pulse mb-4"
        />
  ...
  return (
    <div className="h-[84dvh] md:h-[90dvh] w-full">
      {/* Top Navigation Bar */}
      <div className="h-[10dvh] sticky top-0 z-40 bg-(--gl-bg-base)/90 backdrop-blur-md border-b border-(--gl-border-light) px-5 sm:px-8 py-4  flex items-center justify-between">
        <h1 className="text-2xl md:text-3xl font-integral font-bold text-(--gl-text-main) mb-2">
          Local Alerts
        </h1>
      </div>
      <div className=" mx-auto w-full p-4 md:p-8 font-satoshi text-(--gl-text-main)">
        ...
        <div className="flex flex-col gap-6">
          {alerts.length === 0 ? (
            <div className="bg-(--gl-bg-surface) border border-(--gl-border-light) rounded-2xl p-10 text-center flex flex-col items-center">
              <LuShieldAlert
                size={48}
                className="text-(--gl-status-success) mb-4"
              />
              <h3 className="text-xl font-bold">No Active Alerts</h3>
              <p className="text-(--gl-text-muted) mt-2">
                Your area is currently clear. Any emergency broadcasts in your
                radius will appear here instantly.
              </p>
            </div>
          ) : (
            alerts.map((alert) => (
              <div
                key={alert._id}
                className="bg-(--gl-sos-pulse) border-l-4 border-(--gl-sos-base) rounded-r-2xl p-5 md:p-6 shadow-sm flex flex-col gap-3 transition-all hover:shadow-md"
              >
                <div className="flex justify-between items-start gap-4">
                  <h3 className="text-xl font-bold text-(--gl-sos-base) leading-tight">
                    {alert.title}
                  </h3>
                ...
                {alert.audioUrl && (
                  <div className="mt-2 bg-(--gl-bg-surface) rounded-xl p-3 border border-(--gl-border-light) flex items-center gap-3">
                    <LuVolume2
                      className="text-(--gl-sos-base) shrink-0"
                      size={20}
                    />
                    <audio
                      src={alert.audioUrl}
